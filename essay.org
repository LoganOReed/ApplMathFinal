...
$$ \int_{\partial D} T \frac{\partial u}{\partial \textbf{n}} \, dS = \int_{\partial D} \rho u_{tt} \, dA $$

where T is the constant tension, $\rho$ is the density constant, and $\textbf{n}$ is the outward normal of the boundary $\cite{ryan}$
